    ...
    T: embedded_graphics::DrawTarget<Rgb565>,
{
    ...
    // 62.5ms x 16 = 1000ms = 1s
    let elapsed_s = counter as f32 / 16.0;
    let mut text_buffer = String::<U256>::new();
    write!(&mut text_buffer, "{:.2}", elapsed_s).unwrap();

    // TODO: タイムスタンプを描画する
    let length = text_buffer.len();
    let left = SCREEN_WIDTH - (length as i32) * FONT_WIDTH;
    egtext!(
        text = text_buffer.as_str(),
        top_left = (left, 0),
        ...
        match state {
            ...
            State::Running => {
                if button_stop.is_low().unwrap() {
                    ...
                    beep(&mut buzzer, &mut delay, 880.hz(), 50_u16);
                    delay.delay_ms(50_u16);
                    beep(&mut buzzer, &mut delay, 880.hz(), 100_u16);
                    state = State::Idle;
                }
                draw(&mut display).unwrap();
            }
